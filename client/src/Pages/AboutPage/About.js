    ...
    async function createItem() {
        if (typeof window.ethereum !== "undefined") {
            ...
            try {
                ...
                web.eth.getAccounts().then(async (account) => {
                    console.log("This should be something", account[0]);

                    let contract = await new web.eth.Contract(KiFTabi, "0x0f2AA31D60C2A9741aFb543340A3a5Bc214c99fd");

                    console.log("sellitem.contract_address is ====>", sellitem.contract_address);
                    console.log("sellitem.NFT_Token_id is ====>", sellitem.NFT_Token_id);
                    console.log("price user have put ====>", web.utils.toWei(String(priceSellerPut), "ether"));
